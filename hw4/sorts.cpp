...
int main()
{
      ...
      // destruction time.
    {
	 ...
	 auxSensorsPtr.resize(auxSensors.size());
	 auto j = auxSensorsPtr.begin();

	 for (auto i =&auxSensors.front(); i != &auxSensors.back() + 1; i++)
	 {
		 *j = i;
		 j++;
	 }
	 ...
	 std::vector<Sensor>::iterator k = sensors.begin();
	 for (std::vector<Sensor*>::iterator i = auxSensorsPtr.begin(); i != auxSensorsPtr.end(); i++)
	 {
		 *k = **i;
		 k++;
	 }
